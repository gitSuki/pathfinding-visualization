...
function runDjikstra(grid) {
  const startNode = grid.cells[store.getters.getStartNode.row][store.getters.getStartNode.col]
  const endNode = grid.cells[store.getters.getEndNode.row][store.getters.getEndNode.col]
  djikstrasAlgo(grid, startNode, endNode)
}
